...
* **`part3_ecs_fargate/`**: Deploys the Frontend and Backend as highly available Docker containers using AWS ECS (Fargate), ECR, and an Application Load Balancer (ALB).
...
2. **AWS CLI:** Authenticated with an IAM User/Role that possesses sufficient permissions (`AdministratorAccess` or full permissions for VPC, EC2, ECS, ECR, ALB, and IAM).
...
### Part 3: ECS Fargate with ALB & Docker
